...
int main (int argc, char* argv[]) {
    ...
    char* pseudo = (char*) malloc(TAILLEMAX*sizeof(char));
    if(pseudo == NULL) exit(1);
    fgets(pseudo, TAILLEMAX, stdin);
    pseudo[strlen(pseudo)-1] = '\0';
    // Création du plateau de jeu
    // TODO (appeler creerPlateau)
    char** plateau = creerPlateau(nl, nc);
    if(plateau == NULL) exit(1);
    // Initialisation du plateau en plaçant nl*nc/3-1 rochers aléatoires 'R'
    time_t t;
    srand((unsigned) time(NULL));
    // TODO (appeler initPlateau)
    initPlateau(plateau, nl, nc, (nl*nc/3)-1);
    afficherPlateau(plateau, nl, nc);
    // Boucle de jeu
    // TODO
    int nbBombes = 0, nbRochersDetruits = 0;
    while (nl >= 3 && nc >= 3
        && nbRochers(plateau, nl, nc)>0
        && nbRochers(plateau, nl, nc)<nl*nc) {
        ...
        if(plateau[l][c] == ' '){
            ...
            int nbAvantBoom = nbRochers(plateau, nl, nc);
            supprimerLigne(&plateau, &nl, nc, l);
            supprimerColonne(&plateau, nl, &nc, c);
            int nbApresBoom = nbRochers(plateau, nl, nc);
            nbRochersDetruits += nbAvantBoom - nbApresBoom;
            printf("Après explosion de la bombe:\n");
            afficherPlateau(plateau, nl, nc);
        }
        else printf("La case est déja occupée\n");     
    }
       
    // Affichage des statistiques
    // TODO
    if(nbRochers(plateau, nl, nc)==0)
        printf("Bravo %s, tu as gagné !\n", pseudo);
    else
        printf("Perdu !\n");
    printf("Nombre de bombes placées par le joueur: %d", nbBombes);
    printf("\n");
    printf("Nombre de rochers détruits: %d", nbRochersDetruits);
    printf("\n");
    printf("Nombre de rochers restant sur le plateau de jeu: %d", nbRochers(plateau, nl, nc));
    printf("\n");
    free(pseudo);
    for (int i = 0; i < nl; ++i)
        free(plateau[i]);
...
char** creerPlateau (int n, int m) {
    // TODO
    char** plateau = (char**) malloc(n*sizeof(char*));
    if(plateau == NULL) return NULL;
    for (int i = 0; i < n; ++i)
    {
        plateau[i] = (char*) malloc(m*sizeof(char));
        if(plateau == NULL) return NULL;
    }
...
void placerUnRocher (char** p, int n, int m) {
    ...
    while(true){
        int randLigne = entierAleatoire(0, n-1);
        int randColonne = entierAleatoire(0, m-1);
        if(p[randLigne][randColonne] == ' '){
            p[randLigne][randColonne] = 'R';
            break;
        }
    }
}
...
bool supprimerLigne(char*** p, int *n, int m, int ligne){
    ...
    {
        // Suppression de la ligne d'indice ligne dans la colonne i
        (*p)[i] = (*p)[i+1];
        // Mise à jour des dimensions physiques de la colonne i
